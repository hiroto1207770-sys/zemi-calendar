...
function aiPlanV50_(b,ctx){
  ...
  var safe=aiVisibleV50_(ctx.state,ctx.me), local=aiLocalAnswerV50_(q,safe);
  ...
  var prompt=aiPromptV50_(q,safe,members,b.history||[]),plan=aiGeminiV50_(prompt);
...
function aiApplyOneV50_(state,ctx,op){
  ...
  if(op.action==='create'){
    ...
    var id='i'+Date.now()+Math.floor(Math.random()*1000),n={id:id,type:op.type||'event',title:op.title,date:op.date,endDate:op.endDate||'',time:op.time||'',endTime:op.endTime||'',pj:op.projectId||'',other:'',team:op.team||'',teamPj:op.team?op.projectId:'',who:op.who||[],visibility:op.visibility||'all',note:op.note||'',tentative:!!op.tentative,remind:op.remind||'',done:false,doneAt:0,deleted:false,createdBy:ctx.me,updatedAt:Date.now(),rep:'none',days:[],until:'',comments:[],skip:[],notes:{}};
    items.push(n);state.items=items;return {ok:true,targetId:id};
  }
  if(op.action==='delete')item.deleted=true;
  else if(op.action==='complete'){if(item.type!=='task')throw new Error('予定は完了にできません');item.done=true;item.doneAt=Date.now();}
...
function aiLocalAnswerV50_(q,s){var today=Utilities.formatDate(new Date(),Session.getScriptTimeZone()||'Asia/Tokyo','yyyy-MM-dd');if(/(件数|いくつ|進捗|遅れ|期限超過)/.test(q)){var open=s.items.filter(function(i){return i.type==='task'&&!i.done;}),late=open.filter(function(i){return i.date&&i.date<today;});return '閲覧できる範囲では、未完了タスクは'+open.length+'件、期限超過は'+late.length+'件です。';}return '';}
function aiPromptV50_(q,s,members,history){var compact={today:Utilities.formatDate(new Date(),Session.getScriptTimeZone()||'Asia/Tokyo','yyyy-MM-dd'),projects:s.projects.map(function(p){return {id:p.id,name:p.name,teams:(p.teams||[]).map(function(t){return t.name;})};}),items:s.items.slice(0,300).map(function(i){return {id:i.id,type:i.type,title:i.title,date:i.date,time:i.time,endTime:i.endTime,pj:i.pj,team:i.team,who:i.who,done:i.done,note:i.note,tentative:i.tentative};}),members:members.map(function(m){return {name:m.name,terms:aiMemberTermsV50_(m)};})};return '水野ゼミの一般ユーザー向けアシスタント。管理者操作、任意コード、未提示データを要求・推測しない。曖昧な名前はneedsClarification=trueで候補提示し操作を空にする。複数予定は1回で全件抽出する。質問:'+q+'\n短期履歴:'+JSON.stringify((history||[]).slice(-8))+'\n許可済みデータ:'+JSON.stringify(compact);}
